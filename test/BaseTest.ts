...
export abstract class BaseTest {
  ...
  protected readonly browserType: "chrome" | "edge" =
    (this as any).testBrowserType || "chrome";

  protected readonly testTimeout: number = (this as any).testTimeout || 0;

  protected readonly keepBrowserOpen: number =
    (this as any).keepBrowserOpen || false;
  ...
  public run(): void {
    const self = this;
    const runnableTests: Array<Function> = getDecoratedProperties(
    ...
    describe(this.name, async function () {
      ...
      afterEach(async function () {
        if (
          !isUndefined(this.currentTest) &&
          (this.currentTest.state != "passed" || this.currentTest.timedOut)
        ) {
      ...
      function getHook(
        hook: "after" | "afterEach" | "before" | "beforeEach"
      ): Function {
        const afterHook: Array<Function> = getDecoratedProperties(
          Object.getPrototypeOf(self),
          hook
        );
  ...
  private async saveBrowserLogs(fileName: string): Promise<void> {
    const logEntries: logging.Entry[] = await this.browser.getLogs(
