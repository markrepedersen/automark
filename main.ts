...
export class TestRunner {
  ...
  private getRunnableSpecs(filter: Filter): Array<string> {
    return filter.directories.reduce(
      (prev: Array<string>, directory: string) =>
        this.filterSpecs({
  ...
  private filterSpecs(filter: Filter): Array<string> {
    ...
    if (
      filter.exclusions &&
      (filter.exclusions.directories || filter.exclusions.files)
    ) {
    ...
    if (filter.files) {
      return filter.files.reduce(
        (prev: Array<string>, filename: string, index: number) =>
          this.isNodeSpec(index)
            ? sync(
                filter.recursive ? `**/${filename}.ts` : `${filename}.ts`,
                options
              ).concat(prev)
            : prev,
        []
    ...
    return sync(
      filter.recursive ? `**/*Spec.ts` : `*Spec.ts`,
      options
  ...
  private getExclusions(exclusions: Exclusions): Array<string> {
    return exclusions.directories
      ...
      .concat(exclusions.files.map((file: string) => `**/${file}.ts`));
  }
}
