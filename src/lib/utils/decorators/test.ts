...
export namespace Test {
    ...
    export function testable(options: TestableOptions): (constructor: Function) => void {
        return function (constructor: Function): void {
            ...
            if (options.testUrl) {
                setBrowserTestableInfo(constructor.prototype, 'testUrl', options.testUrl);
            }
            if (options.useReverseProxy) {
                if (config.reverseProxyUrl) {
                    setBrowserTestableInfo(constructor.prototype, 'testUrl', config.reverseProxyUrl);
                } else {
                    throw Error('Reverse proxy was not specified in the build parameters');
                }
            }
            if (options.testUsername) {
                setBrowserTestableInfo(constructor.prototype, 'testUsername', options.testUsername);
            }
            if (options.testPassword) {
                setBrowserTestableInfo(constructor.prototype, 'testPassword', options.testPassword);
            }
        };

        function setBrowserTestableInfo(target: any, propertyKey: string, value: any): any {
