...
export abstract class Browser {
    ...
    public async waitForUrlToContainValue(value: string): Promise<void> {
        await this.waitFor(async () => (await this.getCurrentUrl()).includes(value));
    }
    ...
    public async clearChromeCache(): Promise<void> {
        ...
        const clearCacheButton: Button = await this.waitUntilClickable(() => this.findElement('* /deep/ #clearBrowsingDataConfirm'));
        await clearCacheButton.click();
        await this.waitUntilAppearanceAndDisappearanceOfElement('* /deep/ #spinnerContainer.active');
        await this.close();
        const tabs: Array<string> = await this.driver.getAllWindowHandles();
    ...
    private getBrowserArguments(): Array<string> {
        ...
        if (this.maximized) {
            ...
            console.log(`[INFO] Using screen resolution of ${this.width}x${this.height}.`);
     ...
     * @returns {Promise<void>}
     */
    public async waitUntilPageHasLoaded<T extends Page>(page: NewablePage<T>): Promise<T> {
     ...
     * @param pages
     */
    public async waitUntilAnyPageHasLoaded<T extends Page>(...pages: NewablePage<T>[]): Promise<void> {
        await this.waitForAny(...pages.map((page: NewablePage<T>) => pageHasLoaded(page)));
    }
     ...
     */
    public async findElement<T extends WebComponent>(selector: string, ComponentType?: { new(element: WebElement, selector: string, parentElement?: WebElement, ...args: any[]): T }): Promise<T> {
        const element: WebElement = await this.driver.findElement(WebComponent.convertSelector(selector));
        return ComponentType ? new ComponentType(element, selector) : new WebComponent(element, selector) as T;
    }
     ...
     */
    public async findElements<T extends WebComponent>(selector: string, ComponentType?: { new(element: WebElement, selector: string, parentElement?: WebElement, ...args: any[]): T }): Promise<Array<T>> {
        return (await this
            .driver
            .findElements(WebComponent.convertSelector(selector)))
            .map((element: WebElement) => ComponentType ? new ComponentType(element, selector) : new WebComponent(element, selector) as T);
    }
     ...
     */
    public async exists<T extends WebComponent>(selector: string): Promise<boolean> {
     ...
     * @returns {Promise<void>}
     */
    public async typeToFocusedElement(...words: Array<string | IKey>): Promise<void> {
        await this.driver.actions().sendKeys(Key.chord(...words)).perform();
    }
     ...
     * @returns {Promise<void>}
     */
    public async waitUntilElementDisappears(selector: string, parent?: string): Promise<void> {
     ...
     * @returns {Promise<void>}
     */
    public async waitUntilAppearanceAndDisappearanceOfElement(selector: string, parent?: string): Promise<void> {
        const locator: string = parent ? `${parent} ${selector}` : selector;
     ...
     * @returns {Promise<void>}
     */
    public async waitForElementToBeOnTop<T extends WebComponent>(lowerZIndex: string, higherZIndex: string): Promise<void> {
     ...
     * @param attributes
     */
    public async waitForElementToHaveAttributes(elementSelector: string, ...attributes: string[]): Promise<void> {
        await this.waitFor(async () => {
     ...
     * @param attributes
     */
    public async waitForElementToNotHaveAttributes(elementSelector: string, ...attributes: string[]): Promise<void> {
        await this.waitFor(async () => {
            const element: WebComponent = await this.findElement(elementSelector);
            return !(await element.hasAttribute(...attributes));
        });
     ...
     * @param classes
     */
    public async waitForElementToHaveClass(elementSelector: string, ...classes: string[]): Promise<void> {
        await this.waitFor(async () => {
            const element: WebComponent = await this.findElement(elementSelector);
     ...
     * @param classes
     */
    public async waitForElementToNotHaveClass(elementSelector: string, ...classes: string[]): Promise<void> {
        await this.waitFor(async () => {
            const element: WebComponent = await this.findElement(elementSelector);
            return !(await element.hasClass(...classes));
        });
     ...
     * @param styles
     */
    public async waitForElementToNotHaveStyle(elementSelector: string, ...styles: string[]): Promise<void> {
        await this.waitFor(async () => {
            const element: WebComponent = await this.findElement(elementSelector);
     ...
     * @param styles
     */
    public async waitForElementToHaveStyle(elementSelector: string, ...styles: string[]): Promise<void> {
        await this.waitFor(async () => {
            const element: WebComponent = await this.findElement(elementSelector);
            return await element.hasStyle(...styles);
        });
     ...
     */
    public async waitForElementToAppear<T extends WebComponent>(locator: string, ComponentType?: { new(element: WebElement, selector: string, ...args: any[]): T }): Promise<T> {
        await this.waitUntilElementIsVisible(() => this.findElement(locator));
     ...
     */
    public async waitForElements<T extends WebComponent>(locator: string, ComponentType?: { new(element: WebElement, selector: string, ...args: any[]): T }): Promise<Array<T>> {
        await this.waitUntilElementIsVisible(() => this.findElement(locator));
     ...
     */
    public async waitForAnyElementToBeVisible<T extends WebComponent>(...locators: string[]): Promise<T | undefined> {
        const conditions: WaitCondition[] = locators.map(locator => elementIsVisible(() => this.findElement(locator)));
     ...
     * @returns {Promise<WebComponent>}
     */
    public async findAny<T extends WebComponent>(...locators: string[]): Promise<T | undefined> {
        const elements: Promise<T>[] = Array
            .from(locators)
            .map(locator => this.findElement(locator));
     ...
     * @param {string} text
     */
    public async waitForElementWithText<T extends WebComponent>(text: string): Promise<T> {
     ...
     * @returns {Promise<void>}
     */
    public async waitUntilClickable<T extends WebComponent>(clickable: () => AwaitableWebComponent<T>): Promise<AwaitableWebComponent<T>> {
     ...
     * @returns {Promise<void>}
     */
    public async waitUntilStale<T extends WebComponent>(stale: () => AwaitableWebComponent<T>): Promise<void> {
     ...
     * @returns {Promise<void>}
     */
    public async waitUntilElementIsNotVisible<T extends WebComponent>(element: () => AwaitableWebComponent<T>): Promise<void> {
     ...
     * @returns {Promise<void>}
     */
    public async waitUntilElementIsVisible<T extends WebComponent>(element: () => AwaitableWebComponent<T>): Promise<void> {
    ...
    public async showCursor(): Promise<void> {
        const script: string = '// Create mouse following image.\n' +
     ...
     * @returns {Promise<WebComponent>}
     */
    private async asyncFind<T extends WebComponent>(elements: Promise<T>[]): Promise<T | undefined> {
